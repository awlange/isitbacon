...
def load_dataset():
    ...
    # Read in data
    X_train = np.fromfile("generated_data/X_train.dat", dtype=np.float32)
    X_val = np.fromfile("generated_data/X_val.dat", dtype=np.float32)
    X_test = np.fromfile("generated_data/X_test.dat", dtype=np.float32)
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.Conv2DLayer(
            network, num_filters=48, filter_size=(24, 24),
...
def main(num_epochs=3, read_params=False):
    ...
    if read_params:
        ...
        loaded_params = np.load('model_baconnet.npz')
    ...
    updates = lasagne.updates.sgd(loss, params, learning_rate=0.00005)
    ...
    np.savez('model_baconnet.npz', lasagne.layers.get_all_param_values(network))
