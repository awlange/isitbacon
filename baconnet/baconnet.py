...
def load_dataset():
    ...
    X_train = X_train.reshape((-1, 4, 64, 64))
    X_val = X_val.reshape((-1, 4, 64, 64))
    X_test = X_test.reshape((-1, 4, 64, 64))
...
def build_cnn(input_var=None):
    ...
    network = lasagne.layers.InputLayer(shape=(None, 4, 64, 64),
                                        input_var=input_var)
    ...
    # A fully-connected layer of 64 units with 10% dropout on its inputs:
    network = lasagne.layers.DenseLayer(
            lasagne.layers.dropout(network, p=.1),
            num_units=64,
            nonlinearity=lasagne.nonlinearities.rectify)

    # And, finally, the 3-unit output layer with 10% dropout on its inputs:
    network = lasagne.layers.DenseLayer(
            lasagne.layers.dropout(network, p=.1),
            num_units=3,
